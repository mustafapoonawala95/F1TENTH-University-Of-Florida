...
class pure_pursuit_controller {
    ...
    geometry_msgs::PointStamped lingering_lookahead_point_map_frame;
    ...
    int iterations_to_wait;
    int iterations_waited;
    ...
    void car_pose_callback(const geometry_msgs::PoseStamped::ConstPtr& car_pose){
     // Pure pursuit algorithm implemented here.
        if(path_detected){
            ...
            // Using the old lookahead point to calculate steering angle for "iterations_to_wait" number of times.
            if(iterations_waited < iterations_to_wait){
                std::cout << "Using old lookahead point. " << iterations_waited << "\n";
                lingering_lookahead_point_map_frame;
                geometry_msgs::PointStamped target_point_car_frame;
                tf2::doTransform(lingering_lookahead_point_map_frame, target_point_car_frame, map_to_car_frame);
                ...
                for(int i=path_seq_;i<(path_size_-1); i++){
                    ...
                    if(discriminant < 0){// Just chill
                        ...
                        if(t1 >= 0 && t1 <=1){
                            ...
                            lingering_lookahead_point_map_frame = target_point_map_frame;
