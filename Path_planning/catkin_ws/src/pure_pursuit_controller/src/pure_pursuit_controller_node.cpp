...
Pure pursuit controller node, subscribes to A_star_shortest_path topic to get waypoints.
...
class pure_pursuit_controller {
    ...
    bool is_goal_close(std::pair<float,float> car_position){
        float dx = car_position.first - goal_point_.first;
        float dy = car_position.second - goal_point_.second;
        return (sqrt((dx*dx) + (dy*dy)) <= goal_threshold_distance_);
    }

    pure_pursuit_controller(ros::NodeHandle *nh){
        path_subscriber = nh->subscribe("/A_star_shortest_path", 1, &pure_pursuit_controller::path_subscriber_callback,this);
    ...
    void path_subscriber_callback(const nav_msgs::Path::ConstPtr& A_star_path){
        ...
        path_to_track_map_frame_.poses = temp_poses;
        ...
        std::cout << "path_to_track_map_frame created succesfully.\n";
        path_detected = true;
    }
    ...
    void car_pose_callback(const geometry_msgs::PoseStamped::ConstPtr& car_pose){
        ...
        if(path_detected){
            ...
            std::cout << "Current car pose is: " << car_x << ", " << car_y << "\n"; 
            ...
            if(is_goal_close(std::make_pair(car_x,car_y))){
