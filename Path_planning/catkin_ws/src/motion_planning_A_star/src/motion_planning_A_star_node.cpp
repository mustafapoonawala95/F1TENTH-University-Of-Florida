...
class A_star_planner {
    ...
    A_star_planner(ros::NodeHandle *nh) {
        ...
        car_pose_subscriber = nh->subscribe("/gt_pose", 1, &A_star_planner::car_pose_callback, this);
        destination_subscriber = nh->subscribe("/move_base_simple/goal", 1, &A_star_planner::destination_callback, this);
        path_publisher = nh->advertise<nav_msgs::Path>("/A_star_shortest_path", 1);
    ...
    void publish_path(){
        path_publisher.publish(A_star_path);
    }
    ...
    void create_final_path(){
        ...
        ROS_INFO("Path found using A* algorithm.\n");
    ...
    void destination_callback(const geometry_msgs::PoseStamped::ConstPtr& destination_point){
        ...
        std::vector<float> f_values(rows_*cols_, INF);
                ...
                for(int i=0; i<8; i++)              //Going through the 8 neighbours of current node.
                {
                    ...
                    else
                    {
                        ...
                        if(alt < g_values[get_idx(rr, cc)])
                        {
                            ...
                            f_values[get_idx(rr, cc)] = g_values[get_idx(rr, cc)] + compute_distance(neighbour_index,dest_index);
                            prev[get_idx(rr, cc)] = current_node_index;      // Updating parent of neighbour as the current node.
                            pq.push(std::make_pair(-1*(f_values[neighbour_index]),neighbour_index));  // Pushing current neighbour in pq.
                        }
