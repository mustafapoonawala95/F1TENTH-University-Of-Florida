...
class A_star_planner {
    ...
    std::vector<float> g_values;//(rows_*cols_,INF);
    ...
    A_star_planner(ros::NodeHandle *nh) {
        map_subscriber = nh->subscribe("/map", 1, &A_star_planner::map_callback, this);
        path_publisher = nh->advertise<nav_msgs::OccupancyGrid>("/A_star_shortest_path", 1);
        rows_ = 1;
        cols_ = 1;
        INF = 99999.00;
        dr = {1, -1, 0, 0, 1, 1, -1, -1};
        ...
        g_values.resize(RowColProduct,INF);
        f_values.resize(RowColProduct,INF);
        prev.resize(RowColProduct,99999);
        loop_count = 0;
    }
    ...
    // NEED TO DEFINE A COMPUTE H FUNCTION HERE.
    float compute_h(int r, int c){                      // Euclidian distance.
        float DR = (dest_row - r);
        float DC = (dest_col - c);
    ...
    //================================================================================
        ...
        g_values.resize(rows_*cols_, INF);
        f_values.resize(rows_*cols_, INF);
        ...
        g_values.at(source_index) = 0.0;                                         //Initializing source node distance as 0
        f_values.at(source_index) = 0.0;//g_values[source_index] + compute_h(source_row,source_col);
        pq.push(std::make_pair(-1*(f_values[source_index]),source_index));     // Added source node pair to priority queue.
        ...
        while(!pq.empty()){
            ...
            float current_node_distance = g_values[current_node_index]; //======
            ...
            else
            {
                ...
                for(int i=0; i<8; i++)                                               //to calculate neighbours
                {
                    ...
                    else
                    {
                        ...
                        float neighbour_dist = 1.00;                   // <==== At the moment the default edge 
                        ...
                        if(alt < g_values[get_idx(rr, cc)])
                        {
                            g_values[get_idx(rr, cc)] = alt;
                            f_values[get_idx(rr, cc)] = g_values[get_idx(rr, cc)] + compute_h(rr, cc);
                            prev[get_idx(rr, cc)] = current_node_index;      // Updating parent of neighbour as the current node.
                            pq.push(std::make_pair(-1*(f_values[get_idx(rr, cc)]),get_idx(rr, cc)));  // Pushing current neighbour in pq.
                        }
        ...
        ROS_INFO("Optimal path found using A* algorithm.\n");
...
int main(int argc, char **argv){
    ros::init(argc, argv, "motion_planning_A_star_node");
    ros::NodeHandle nh;
    A_star_planner planner = A_star_planner(&nh);
    ros::Duration(1).sleep();
    ros::spinOnce();

    std::cout << "Waiting for somebody to subscribe to /A_star_shortest_path topic. \n";
    ...
    std::cout << "Somebody subscribed to /A_star_shortest_path topic. \nPublishing one last time after 30 seconds and exiting.\n";
