...
class dijkstra_planner {
    ...
    std::vector<int> distances;//(rows_*cols_,INF);
    ...
    dijkstra_planner(ros::NodeHandle *nh) {
        ...
        path_publisher = nh->advertise<nav_msgs::OccupancyGrid>("/shortest_path", 1);
        rows_ = 1;
        cols_ = 1;
        INF = 99999;
        dr = {1, -1, 0, 0, 1, 1, -1, -1};
        ...
        distances.resize(RowColProduct,INF);
    ...
    //================================================================================
        ...
        while(!pq.empty()){
            std::pair<int,int> current_node = pq.top();
            ...
            else
            {
                ...
                for(int i=0; i<8; i++)                                               //to calculate neighbours
                {
                    ...
                    else
                    {
                        ...
                        int neighbour_dist = 1;                       // <==== At the moment the default edge 
                                                                            // distance between neighbouring cells
                                                                            // is set to 1
                        
                        
                        int alt = current_node_distance + neighbour_dist;      
        ...
        ROS_INFO("Callback ended at time: %f \n", current_time);
...
int main(int argc, char **argv){
    ...
    while(planner.get_subscriber_count()==0){            // Will keep on waiting till Rviz subscribes to /shortest_path topic.
        ...
        std::cout << "Waiting for somebody to subscribe to /shortest_path topic. \n";
        ros::Duration(5).sleep();
    }
    std::cout << "Somebody subscribed to /shortest_path topic. \nPublishing one last time after 30 seconds and exiting.\n";
