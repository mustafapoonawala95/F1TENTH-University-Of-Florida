...
def getRange(data,angle):
	...
	idx_float = ((angle+45.0) / 270.0) * (len(data.ranges) - 1)
...
#		flag_obstacle = 1
	...
	if direction==0:
		error, curr_dist_left=followLeft(data,final_desired_trajectory)
	elif direction==1:
		error, curr_dist_right = followRight(data,final_desired_trajectory)#followRight(data,2.)
	
	elif direction==2:
		error, curr_dist_center = followCentre(data,final_desired_trajectory)#followRight(data,2.)
	else:
		print("No path! stop.")
	"""
